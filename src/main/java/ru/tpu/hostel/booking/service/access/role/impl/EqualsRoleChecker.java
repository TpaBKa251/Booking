...
public class EqualsRoleChecker implements RoleChecker {
    ...
    public boolean isRolePermitted(Roles[] userRoles, Roles neededRole, Roles... permittedRoles) {
        ...
        permittedRolesSet.add(neededRole);

        return Arrays.stream(userRoles).anyMatch(permittedRolesSet::contains);
    }

    @Override
    public boolean isApplicable(RoleCheckStrategy strategy) {
        return strategy == RoleCheckStrategy.EQUALS;
    }
